...
def fp_growth(dataset, min_sup, min_conf):
    ...
    def find_prefix_path(basePat, node):
        condPats = {}
    ...
    def mine_frequent_itemsets(root, _header_table, min_sup, prefix, frequent_itemsets):
    # Iterate over the header table in reverse order
        for item, (count, node) in reversed(list(_header_table.items())):
            ...
            cond_path_bases = find_prefix_path(basePat, _header_table[basePat][1])
